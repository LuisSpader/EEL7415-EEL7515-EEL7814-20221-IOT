...
class myPacket():
    def __init__(self, nodeid, plen, distance, bs):
        ...
        if (experiment == 3) or (experiment == 5):
            ...
            for i in range(0,6):
                for j in range(1,4):
                    if (sensi[i,j] < Prx): #- 7.8): # Pra compensar o sombreamento
                        self.sf = sensi[i,0]
                        if j==1:
                            self.bw = 125
                        elif j==2:
                        ...
                        if at < minairtime:
                            ...
                            minsf = self.sf
                            minbw = self.bw
                            Sensibilidade = sensi[i,1]
                            #minsensi = sensi[í,j]

            self.rectime = minairtime
            self.sf = minsf
            self.bw = minbw
            minsensi = Sensibilidade
            if (minairtime == 9999):
                self.sf = 12
                self.bw = 125
                Sensibilidade =  sensi[5,1]
                #print( "does not reach base station")
                #sys.exit(-1)

        
            if [int(self.sf), float(distance),int(self.bw),minsensi] not in sf_and_distance:
                sf_and_distance.append([int(self.sf), float(distance),int(self.bw),minsensi])
        ...
        self.arriveTime = 0
        self.rssi = Prx #+ np.random.normal(0, 7.8) # Sombreamento
        ...
        #if experiment != 3:
        #if experiment == 3 or experiment == 5:
        #    self.lost = self.rssi < Sensibilidade
        #else:
...
# get arguments
    ...
    if len(_list) >= 6:
        nrNodes = _list[1]
        avgSendTime = _list[2]*1000
        experiment = dict_experiments[_list[3]]
        simtime = _list[4]*60*60*1000
        nrBS = _list[5]
        if len(_list) > 6:
            full_collision = _list[6]
        MaxDist = _list[7]
    ...
    if experiment in [0,1,4]:
        ...
        minsensi = -126.5   # no experiments, so value from datasheet
    elif experiment == [3, 5]:
    ...
    if experiment == 3 or experiment == 5:
        final_sf = [0]*6
        total_sf = 0
        with open('sf_distance.csv', 'w') as nfile:
            nfile.write('SF,Distance,BW,sensi\n')
            for sf_distance in sf_and_distance:
                nfile.write(str(sf_distance[0])+','+str(sf_distance[1])+','+str(sf_distance[2])+','+str(sf_distance[3])+'\n')
                final_sf[sf_possible[sf_distance[0]]] += 1
                total_sf += 1
        with open('sf_pct.csv', 'w') as nfile:
            nfile.write('SF7,SF8,SF9,SF10,SF11,SF12\n')
            nfile.write(str(final_sf[0]/total_sf)+','+str(final_sf[1]/total_sf)+','+str(final_sf[2]/total_sf)+','
                        + str(final_sf[3]/total_sf)+','+ str(final_sf[4]/total_sf)+','+ str(final_sf[5]/total_sf)+'\n')

  #  with open('basestation.csv', 'w') as bfile:
...
#            bfile.write('{x},{y},{id}\n'.format(**vars(basestation)))
    ...
    fname = "exp" + str(undoDict[experiment]) + ".csv"# + "BS" + str(nrBS) + ".csv"
    print(fname)
    if os.path.isfile(fname):
        res = "\n" + str(nrNodes) + "," + str(der) + "," + str(len(collidedPackets)) + ","  + str(packetSeq) + "," + str(len(recPackets)) + "," + str(len(lostPackets))
    else:
        res = "nrNodes, der, nrCollisions, nrTransmissions, nrReceived, nrLost\n" + str(nrNodes) + "," + str(der) + "," + str(len(collidedPackets)) + ","  + str(packetSeq) + "," + str(len(recPackets))+ "," + str(len(lostPackets))
